...
recursos_estagio_1 = vessel.resources_in_decouple_stage(stage=3, cumulative=False)
combustivel1 = conn.add_stream(recursos_estagio_1.amount, 'LiquidFuel')
recursos_estagio_2 = vessel.resources_in_decouple_stage(stage=0, cumulative=False)
...
texto.color = (1, 1, 1)
texto.size = 16
rect.position = (110 - (tamanho_tela[0]/5),0)
def escrever(text):
    texto.content = text
    texto.rect_transform.position = (0, -20)
    texto.color = (1, 1, 1)
...
while altitude_alvo >= apoastro():
    ...
    # Reduzir potência
    if altitude() < 60000:
        if vessel.flight().g_force > 2.:
            vessel.control.throttle = vessel.control.throttle - 0.001
        elif vessel.flight().g_force < 1.5:
            vessel.control.throttle = vessel.control.throttle + 0.001
    else:
        vessel.control.throttle = 1.0
    ...
    if primeiro_estagio_separado == False:
        if combustivel1() == 0.0:
            ...
            vessel.control.activate_next_stage()
            for segundos in range(25, 1, -1):
                escrever('Ligar motor em {}'.format(segundos))
                sleep(1)
            escrever('Motor 2º s')
            vessel.control.activate_next_stage()
            sleep(1)
    ...
    if apoastro() > altitude_alvo * 0.996:
